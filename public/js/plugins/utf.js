...
function showMap(obj) {
  ...
  mapdiv.innerHTML =
    '<table border="0" cellpadding="0" cellspacing="0" style="border:3px solid #0e88af;background-color:#ffffff;width:100%;"><tr>' +
    '<td style="background-color:#0e88af;color:#ffffff;" nowrap="nowrap"><b>&nbsp;Keypad Map - ' +
    lang.substring(0, 1).toUpperCase() +
    lang.substring(1) +
    '</b></td><td bgcolor="#0e88af" nowrap="nowrap" width="20" align="right">' +
    ...
    '<b> &nbsp; X &nbsp; </b></div></td></tr><tr><td colspan="2" align="center"><img name="KeyMap" src=' +
    myimg.src +
...
function moveMap(e) {
  ...
  if (dragok) {
    if (isIE) {
      mapstyle.left = dx + e.clientX - tempX + "px";
      mapstyle.top = dy + e.clientY - tempY + "px";
    } else {
      mapstyle.left = dx + e.pageX - tempX + "px";
      mapstyle.top = dy + e.pageY - tempY + "px";
    }
    return false;
  }
}
...
function isEnglish(elem) {
  var alphaExp = /[a-zA-Z]+/;
  if (elem.match(alphaExp)) return true;
  else return false;
}
...
function convertThis(e, numchar) {
  ...
  if (typeof numchar == "undefined") numchar = 4;
  if (isIE) {
    ...
    if (prevChar.length != 0) document.selection.clear();
    ...
    if (myField.selectionStart >= 0) {
      if (isSplKey(Key) || e.ctrlKey) return true;
      ...
      if (myField.value.length == 0) {
        ...
        myField.value =
          prevStr +
          getLang(prevChar, Char, myField.selectionStart) +
...
function toggleKBMode(e, obj) {
  if (obj != null) {
    ...
    else myimg.src = webhome + "/images/ucedit/" + lang + ".png";
    savepref = eval("document." + obj.form.name + ".savepref");
    if (savepref != null) {
      savepref.disabled = false;
      savepref.onclick = savePref;
...
function savePref(e) {
  if (isIE) savepref = event.srcElement;
  else savepref = e.target;
  ...
  if (tha != null) setCookie(lang + "_tha", tha.checked, 365);
  if (nga != null) setCookie(lang + "_nga", nga.checked, 365);

  for (var i = 0; i < keybrd.length; i++)
    if (keybrd[i].checked) setCookie(lang + "_keybrd", keybrd[i].value, 365);
...
function restorePref() {
  if (
    getCookie(lang + "_help") != null &&
    document.post != null &&
    document.post.showhelp != null &&
    document.post.savepref != null &&
  ...
  if (
    getCookie(lang + "_tha") != null &&
    document.post != null &&
    document.post.tha != null &&
    document.post.savepref != null &&
  ...
  if (
    getCookie(lang + "_nga") != null &&
    document.post != null &&
    document.post.nga != null &&
    document.post.savepref != null &&
  ...
  if (
    getCookie(lang + "_keybrd") != null &&
    document.post != null &&
    document.post.keybrd != null &&
    document.post.savepref != null
  ) {
  ...
  if (
    getCookie(lang + "_showmap") != null &&
    document.post != null &&
    document.post.showmap != null &&
    document.post.savepref != null &&
...
function isSplKey(keynum) {
  retVal = false;
  for (i in SplKeys) {
    if (keynum == SplKeys[i]) retVal = true;
  }
  return retVal;
}
...
function getLang(prv, txt, sP) {
  ...
  if (kbmode == "english") {
    ...
    if (isEnglish(prvPrv)) prv = "";
    if (prv == ugar && mapLang(txt, sP, "tw") == uugar)
      retTxt = mapLang(prv + txt, sP, "tw");
    else retTxt = prv + mapLang(txt, sP, "tw");
    if (isEnglish(prvPrv)) retTxt = prvPrv + retTxt;
    if (lang == "tamil");
    ...
    else retTxt = mapLang(prv + txt, sP, "teap");
    if (isEnglish(prvPrv)) retTxt = prvPrv + retTxt;
  } else if (kbmode == "bamini") {
    if (isEnglish(prvPrv)) prv = "";
    if (prv == ugar && mapLang(txt, sP, "taba") == uugar)
      retTxt = mapLang(prv + txt, sP, "taba");
    else retTxt = prv + mapLang(txt, sP, "taba");
    if (isEnglish(prvPrv)) retTxt = prvPrv + retTxt;
    ...
    else retTxt = prv + mapLang(txt, sP, "tava");
    if (isEnglish(prvPrv)) retTxt = prvPrv + retTxt;
    retTxt = typeOrderFix(retTxt);
    retTxt = canoFix(retTxt);
  } else if (kbmode == "modular") {
    ...
    else retTxt = prv + mapLang(txt, sP, "tamo");
    if (isEnglish(prvPrv)) retTxt = prvPrv + retTxt;
    retTxt = canoFix(retTxt);
  } else {
    if (pkbmode == "english") {
...
function mapLang(txt, sP, mod) {
  ...
  if (mod != null && mod == "tw") txtarr = eval(lang.substring(0, 2) + "tw");
  if (
    mod != null &&
    (mod == "teap" ||
      mod == "ta99" ||
...
function showCombi(e) {
  ...
  else prevWord = prevWord.substring(prevWord.length - 1);

  helptxt = "";
  prevLet = getLang(prevWord, Char, 0);
  prevLet = prevLet.substring(prevLet.length - 1);
  if (
    prevWord != "" &&
    !isLangOtru(prevWord) &&
    prevLet != getLang("", Char, 0)
  ) {
    if (
      Char == "a" ||
      Char == "i" ||
      ...
      helptxt =
        '<td style="font-size:12px;border:1px solid #0DE8E9;">' +
        prevWord +
        " + " +
        Char +
        " = <b>" +
        getLang(prevWord, Char, 0) +
        "</b></td>";
      if (Char == "a")
        helptxt +=
          '<td style="font-size:12px;border:1px solid #0DE8E9;">' +
          prevWord +
          " + i = <b>" +
          getLang(prevWord, "i", 0) +
          "</b></td><td style='font-size:12px;border:1px solid #0DE8E9;'>" +
          prevWord +
          " + u = <b>" +
          getLang(prevWord, "u", 0) +
          "</b></td>";
      ...
      helptxt =
        ...
        prevWord +
        " + a + i = <b>" +
        getLang(getLang(prevWord, "a", 0), "i", 0) +
        "</b></td><td style='font-size:12px;border:1px solid #0DE8E9;'>" +
        prevWord +
        " + o = <b>" +
        getLang(prevWord, "o", 0) +
        "</b></td><td style='font-size:12px;border:1px solid #0DE8E9;'>" +
        prevWord +
        " + o = <b>" +
        getLang(prevWord, "O", 0) +
        ...
        prevWord +
        " + a + u = <b>" +
        getLang(getLang(prevWord, "a", 0), "u", 0) +
        "</b></td>";
      if (lang == "tamil") {
        ...
        if (getLang("", "S", 0) == prevWord)
          helptxt +=
            ...
            prevWord +
            " + r + I = <b>" +
            getLang(getLang(prevWord, "r", 0), "I", 0) +
            "</b></td>";
        if (getLang("k", "n", 0).indexOf(prevWord) > 0)
          helptxt +=
            ...
            prevWord +
            " + t + h = <b>" +
            getLang(getLang(prevWord, "t", 0), "h", 0) +
            "</b></td><td style='font-size:12px;border:1px solid #0DE8E9;'>" +
            prevWord +
            " + g = <b>" +
            getLang(prevWord, "g", 0) +
            "</b></td><td style='font-size:12px;border:1px solid #0DE8E9;'>" +
            prevWord +
            " + j = <b>" +
            getLang(prevWord, "j", 0) +
            "</b></td>";
      }
    }
    helpdiv.innerHTML =
      '<table cellpadding="2" cellspacing="0" border="0" style="border:1px solid #0DE8E9;background-color:#BDE8E9"><tr>' +
      helptxt +
      "</tr></table>";
    helpstyle.display = "block";
  } else helpstyle.display = "none";
...
function isLangOtru(letter) {
  ...
  for (i = 0; i < otruArr.length; i++) if (otruArr[i] == letter) isOtru = true;
  return isOtru;
}
...
function showHelp(obj) {
  ...
  helpstyle = getStyleObject("HelpDiv");
  if (!toShowHelp) helpstyle.display = "none";
}
...
function toggleScreen() {
  ...
  if (styleObj.paddingTop == "" || styleObj.paddingTop == "156px") {
    ...
    document.getElementById("updownarrow").src =
      webhome + "/templates/siteground39/images/downarrow.gif";
    setCookie("ScreenPref", "Half", 365);
  } else {
    styleObj.paddingTop = "156px";
    document.getElementById("updownarrow").src =
      webhome + "/templates/siteground39/images/uparrow.gif";
